...
| 3. Unauthorized, low-trust, stale-member, and cross-company resolution is denied without a wake | Task 3 (named-user / override / member-role / stale-member / no-wake tests); low-trust: Task 1's `"keeps the low-trust control-plane denial…"` case + the frozen LT-26 floor (`server/src/__tests__/low-trust-red-team-routes.test.ts:983–1046`); cross-company/agent/viewer: frozen `collab-invariants-routes.test.ts` cases |
