...
- A **non-default** policy (`responsible_user` / `typed_execution_participant`) narrows resolution to: **(a)** the named `userId`, and **(b)** board principals as an audited override. "Board principal" is defined as: `local_implicit` board actor (single-operator instance), `isInstanceAdmin`, or active membership with role `owner` or `admin` (`COMPANY_MEMBERSHIP_ROLES`, `packages/shared/src/constants.ts:814–821`). Active members with role `operator` who are not the named user are denied — this is the deliberate flip of the Stage 0 characterization (Task 3).
...
|---|---|---|---|
| Pending package-bearing interactions per issue | **3** | create transaction count, `status = 'pending' AND jsonb_exists(payload, 'decisionPackage')` | `422 { code: "DECISION_PACKAGE_PENDING_ISSUE_LIMIT" }` |
| Pending package-bearing interactions per company | **100** | same, company scope | `422 { code: "DECISION_PACKAGE_PENDING_COMPANY_LIMIT" }` |
...
- **Attention route query params (first params ever beyond `includeDismissed`, `server/src/routes/attention.ts:10–25`):** `limit` (int, 1–200) and `cursor` (opaque base64url of `${activityAtIso}:${itemId}` taken from the previous page's last item). Pagination engages when either param is present; a param-less request keeps today's full response byte-compatible with the Stage 0 contract freeze. When engaged, items sort strictly by `(activityAt DESC, id DESC)`, the response envelope gains `nextCursor: string | null`, and `limit` defaults to 50 when only `cursor` is supplied. `rank` remains presentation-only (extension-points §3: copy/rank are not schema); pagination changes nothing about entry/exit rules.
- **Supersession:** a newer package-bearing interaction on the same `(issue, kind, target)` — target = the `requestConfirmationTarget` `key` + `revisionId` when present, else issue-level — expires the older pending one **in the same create transaction**, via the existing supersession lifecycle (status `expired`); the only additive schema change is the new result reason member `"superseded_by_interaction"` alongside today's `"superseded_by_comment"` (`requestConfirmationResultSchema.outcome`, `issue.ts:870–877`; `askUserQuestionsResultSchema.expirationReason`, `issue.ts:708`). No new supersession mechanism (Q13).
...
- Modify: `server/src/routes/issues.ts:6745` (`DELETE /work-products/:id`: pending-package guard) and the issue-attachment delete route (same guard)
...
- Modify: `packages/shared/src/validators/issue.ts` (`requestConfirmationResultSchema.outcome` and `askUserQuestionsResultSchema.expirationReason` gain `"superseded_by_interaction"`); mirror in `packages/shared/src/types/issue.ts`
...
- [ ] **Step 1: Write failing bounds tests** (`decision-package-bounds-routes.test.ts`):
  ...
  - `"does not supersede across different kinds or targets"` — different `kind` or different `target.revisionId` → both pending (both count against the cap).
  - `"survives concurrent duplicate creates without exceeding the cap"` — two parallel creates at the cap boundary → exactly one 201 (transactional count; the partial index backs it).
...
- [ ] **Step 3: Run to verify failure, implement** — caps + supersession inside the create transaction (count → 422 before insert; supersession `UPDATE … SET status = 'expired', result = …` before insert, plus an `issue.thread_interaction_expired` activity row with `details.supersededByInteractionId`); pagination per §4 (sort `(activityAt DESC, id DESC)` when `page` present, slice `limit`, `nextCursor` from the last emitted item). `pnpm db:generate` + `pnpm --filter @paperclipai/db check:migrations` for the index.
...
- [ ] **Step 2: Amend the budget characterization** (`collab-invariants-routes.test.ts`, "budget hard-stop blocks collab-triggered continuation wake") — **deliberate flip**, replacing the `// Stage 1 changes this deliberately: roadmap §4.1 #14 / continuation-outbox plan Track A…` marker:
  ...
  - `"blocks heartbeatService(db).wakeup directly with the budget reason"` — **unchanged** (the budget gate itself is untouched).
...
| same | `"blocks heartbeatService(db).wakeup directly with the budget reason"`, XOR, boundary, viewer, agent-403 cases | **Unchanged** | — |
