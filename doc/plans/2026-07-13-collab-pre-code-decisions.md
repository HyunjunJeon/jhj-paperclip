...
**Status:** PROPOSED (eng) — the governing constraints are ratified here; the concrete per-state actor/effect matrix is a separate deliverable (Task B7), not re-derived in this document
...
**Status:** PROPOSED (eng) — evaluation order is ratified by the roadmap; the storage backend choice is what this entry closes
...
**Status:** PROPOSED (eng) — the `resolverPolicy` mechanism itself is already ratified in the roadmap's Stage 1 shape; this entry closes the open default and non-owner behavior
...
**Status:** PROPOSED (eng) — the bounding principles (server-derived idempotency, supersession, pagination) are ratified by the roadmap; the numeric caps below are what this entry closes
...
| 13 | Decision queue bounds | decide | PROPOSED (eng) | eng-ratify |
| 14 | Outcome authority | ratify | PROPOSED (eng) | eng-ratify |
| 15 | Steer delivery | ratify | PROPOSED (eng) | eng-ratify |
| 16 | Command composer | ratify | PROPOSED (eng) | eng-ratify |
